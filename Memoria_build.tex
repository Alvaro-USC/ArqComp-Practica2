...
Para $n{=}3200$, la versión SIMD (v4) alcanza un \textit{speedup} de \textbf{1.45$\times$} sobre v2\texttt{-O3}, mientras que la mejor configuración OpenMP (v3, \texttt{static}, 32 hilos) consigue \textbf{25.43$\times$} respecto a v3 con 1 hilo. Los experimentos muestran que el escalado OpenMP depende del tamaño: a 32 hilos, v3 pasa de \textbf{25.25$\times$} en $n{=}1250$ a \textbf{25.43$\times$} en $n{=}3200$.\\[2pt]
...
% AUTO_G1_TEXT_BEGIN
La Figura~\ref{fig:g1} muestra el \textit{speedup} de v2 respecto a v1 con ambas versiones en \texttt{-O0}. Los valores medidos son $n=1250$: \textbf{1.16$\times$}, $n=2000$: \textbf{1.09$\times$} y $n=3200$: \textbf{0.97$\times$}. El efecto depende del tamaño: algunas configuraciones mejoran frente a v1 y otras quedan por debajo.
...
% AUTO_G2_TEXT_BEGIN
La Figura~\ref{fig:g2} compara la versión secuencial optimizada (v2), la mejor configuración OpenMP (v3) y la versión SIMD (v4) respecto a v1\texttt{-O3}. Los speedups de v2 son $n=1250$: \textbf{1.73$\times$}, $n=2000$: \textbf{1.65$\times$} y $n=3200$: \textbf{1.05$\times$}; los de v3, $n=1250$: \textbf{25.88$\times$}, $n=2000$: \textbf{29.78$\times$} y $n=3200$: \textbf{25.69$\times$}; y los de v4, $n=1250$: \textbf{1.70$\times$}, $n=2000$: \textbf{1.61$\times$} y $n=3200$: \textbf{1.52$\times$}. En estos resultados, \textbf{v3} es la serie más rápida en 3/3 tamaños. Las mejores configuraciones de v3 son $n=1250$: \texttt{static} con 32 hilos, $n=2000$: \texttt{static} con 32 hilos, $n=3200$: \texttt{static} con 32 hilos.
...
% AUTO_G3_TEXT_BEGIN
La Figura~\ref{fig:g3} muestra el \textit{speedup} de la mejor configuración OpenMP (v3) y de la versión SIMD (v4) respecto a v2\texttt{-O3}. OpenMP obtiene speedups entre \textbf{14.93$\times$} y \textbf{24.51$\times$}, mientras que SIMD logra entre \textbf{0.98$\times$} y \textbf{1.45$\times$}. En conjunto, \textbf{v3} aporta la mayor ganancia respecto a la mejor versión secuencial.
...
% AUTO_G4_TEXT_BEGIN
La Figura~\ref{fig:g4} es la gráfica \textbf{obligatoria} del enunciado y resume el speedup de v3\texttt{-O3} con \texttt{schedule(static)} frente a 1 hilo. A 32 hilos, los speedups alcanzan $n=1250$: \textbf{25.25$\times$}, $n=2000$: \textbf{29.25$\times$} y $n=3200$: \textbf{25.43$\times$}. El mejor escalado se observa en $n=2000$, mientras que $n=1250$ es el caso más sensible al overhead de sincronización.
...
% AUTO_G5_TEXT_BEGIN
La Figura~\ref{fig:g5} compara los tres modos de reparto de OpenMP sobre v3. A 32 hilos, la mejor política para cada tamaño es $n=1250$: \texttt{static} con \textbf{25.25$\times$}, $n=2000$: \texttt{static} con \textbf{29.25$\times$}, $n=3200$: \texttt{static} con \textbf{25.43$\times$}.
...
% AUTO_G6_TEXT_BEGIN
La Figura~\ref{fig:g6} compara la variante base con \texttt{reduction(+:norm2)} frente a la variante con \texttt{critical}. A 32 hilos, se obtiene $n=1250$: \texttt{reduction}=25.25$\times$, \texttt{critical}=21.61$\times$; $n=2000$: \texttt{reduction}=29.25$\times$, \texttt{critical}=27.08$\times$; $n=3200$: \texttt{reduction}=25.43$\times$, \texttt{critical}=25.24$\times$. La mayor diferencia relativa aparece en $n=1250$, donde \texttt{reduction} aventaja a la otra opción en aproximadamente \textbf{14.4\%}.
...
\begin{enumerate}
  \item Las optimizaciones de caché de v2 frente a v1 con \texttt{-O0} producen speedups entre \textbf{0.97$\times$} y \textbf{1.16$\times$}.
  \item La versión SIMD (v4) mejora a v2\texttt{-O3} con speedups entre \textbf{0.98$\times$} y \textbf{1.45$\times$}.
  \item Con \texttt{schedule(static)}, v3 alcanza $n=1250$: \textbf{25.25$\times$}, $n=2000$: \textbf{29.25$\times$} y $n=3200$: \textbf{25.43$\times$} a 32 hilos.
  ...
  \item La comparación \texttt{reduction} vs. \texttt{critical} en $n=3200$ y 32 hilos da \textbf{25.43$\times$} frente a \textbf{25.24$\times$}.
\end{enumerate}
